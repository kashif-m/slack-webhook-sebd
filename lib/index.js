...
const getPayload = async (args, buttons) => {
	...
	while(args[0]) {
		...
		if(heading.match(/(repository|commit|version|release scope|build number|development build|app name|app id|source branch|destination branch)/i)) {

			value = heading.match(/(development build|build number)/i) ? value
			...
			if(heading.match(/pull request|triggered by/i)) {
				blocks.push({
					"type": "section",
					"text": {
						"type": "mrkdwn",
				...
				sliceVal = 3
			} else if(heading.match(/jira project/i))
				blocks.push({
					"type": "section",
					"text": {
						"type": "mrkdwn",
